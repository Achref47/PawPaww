...
const ChatSocketHelper = {
  onConnection(socketIO, app) {
    app.get(`/user/:UID1/chat/:UID2`, async (req, res)=>{
      ...
      console.log("will lookup chat between ", UID1, UID2)
    ...
    app.get('/chats/:UID', async (req, res) => {
      ...
      users = users.map(user => ({
        UID: user.userId, username: user.username, isOnline: getOnlineStatusForUID(user.userId)
      }));
      console.log("will send users:", users);
